function [  effic_eMBB_time_avg, covrg_time_avg] =   cp_urllc_one_trial(...
                                    lr_theta, ...
                                    p_plus_pop,...
                                    p_minus_pop,...
                                    p_plus_hat,...
...
for ff=2:num_frames
    ...
    q(1+neighbor_p1_int) =              p_plus_hat *(G_num_fminus1<G_num_max_hat) / length(neighbor_p1_int) ;      % birth      w.p. p_plus_hat
    q(1+neighbor_m1_int) =              p_minus_hat*(G_num_fminus1>G_num_min_hat) / length(neighbor_m1_int) ;      % death      w.p. p_minus_hat
    q(1+neighbor_00_int) =              (1 - p_plus_hat  * (G_num_fminus1<G_num_max_hat) ...
    ...
    var_phi_f =                         varphi(theta_f(ff)); % stretching func
    sorted_idx =                        sort_random_tiebreakers(q,'descend');
    cdf_vec =                           cumsum(q(sorted_idx));
    if var_phi_f==0.0
        U_alc_f(ff,:) =                 ones(1,S); % technical condition. if theta hits 1, we must have the entire set within
    else
        pred_set_len =                  min(find(cdf_vec>=1-var_phi_f));
        ...
        rand_cutoff =                   (p_below + (p_above-p_below) * rand(1)) < (1-var_phi_f); % 0 or 1
...
effic_eMBB_time_avg =                   effic_eMBB_time_avg(end);
covrg_time_avg =                        covrg_time_avg(end);
